...
class _MyAssetAccumState extends State<MyAssetAccumBody> {
  ...
  Widget bottomTitles(double value, TitleMeta meta) {
    ...
    return Padding(
      child: Text(text.substring(0, 4) + '.' + text.substring(4, 6), style: style),
  ...
  Widget build(BuildContext context) {
    ...
    strtDt = dateUtils.DateToYYYYMMDD(dateUtils.stringToDateTime(strtDt).subtract(const Duration(days: 120)));
      ...
      builder: (BuildContext context, AsyncSnapshot snapshot) {
        ...
        // 데이터를 정상적으로 받아오게 되면 다음 부분을 실행하게 되는 것이다.
          ...
          for(var i = 0; i < snapshot.data.length; i++) {
            ...
            _groupDataList.add(generateGroupData(i, snapshot.data[i]['data']));
            assetInfo = _calculateAssetInfo(assetInfo, snapshot.data[i]['data']);
          }
  ...
  Map<String, dynamic> _calculateAssetInfo(Map<String, dynamic> assetInfo, List<dynamic> element) {
    for(int i = 0; i < element.length; i++) {
      ...
      if ( dataLength > 0 ) {
        totalSumPriceDiff = element[i]['total_sum_price'] - assetInfo[element[i]['asset_id']]['data'][dataLength - 1]['total_sum_price'];
        totalSumPriceDiffPercentage = (element[i]['total_sum_price'] - assetInfo[element[i]['asset_id']]['data'][dataLength - 1]['total_sum_price']) / assetInfo[element[i]['asset_id']]['data'][dataLength - 1]['total_sum_price'] * 100;
      }
      assetInfo[element[i]['asset_id']]['data'].add({
        'accum_dt': element[i]['accum_dt'],
        'total_sum_price': (element[i]['total_sum_price'] is double) ? element[i]['total_sum_price'].toInt() : element[i]['total_sum_price'],
  ...
  List<Widget> _makeListView(Map<String, dynamic> assetInfo) {
    ...
    _listViewList.add(
      Text(assetInfo['asset_nm'], style: TextStyle(fontWeight: FontWeight.bold))
    );
    _listViewList.add(
    ...
    for (int i = 0 ; i < assetInfo['data'].length ; i++) {
      _listViewList.add(
        Row(
          ...
          children: [
            Text(assetInfo['data'][i]['accum_dt'].substring(0, 4) + '년 ' + assetInfo['data'][i]['accum_dt'].substring(4, 6) + '월'),
            Column(
              crossAxisAlignment: CrossAxisAlignment.end,
              children: [
                Text(
                  numberUtils.comma(assetInfo['data'][i]['total_sum_price']),
                  style: TextStyle(
                    color: getTextColor(assetInfo['data'][i]['total_sum_price_diff']),
                ...
                Text( 
                  ' ( ' + 
                  numberUtils.comma(assetInfo['data'][i]['total_sum_price_diff']) +
                  ' )' +
                  ' ( ' + 
                  assetInfo['data'][i]['total_sum_price_diff_percentage'].toStringAsFixed(2) + '%' +
                  ' )',
                  style: TextStyle(
                    color: getTextColor(assetInfo['data'][i]['total_sum_price_diff']),
                    fontWeight: FontWeight.bold,
