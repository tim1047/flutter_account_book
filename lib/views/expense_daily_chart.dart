...
class ExpenseDailyChartBodyState extends State<ExpenseDailyChartBody> {
  ...
  Widget build(BuildContext context) {
    ...
    final List<Color> _indicatorColors = [
      Color(0xff4af699), Color.fromARGB(255, 37, 205, 243), Color.fromARGB(255, 227, 47, 11),
    ];
      ...
      builder: (BuildContext context, AsyncSnapshot snapshot) {
        ...
        // 데이터를 정상적으로 받아오게 되면 다음 부분을 실행하게 되는 것이다.
          ...
          LineChartData sampleData1 = LineChartData(
            ...
            showingTooltipIndicators: showIndexes.map((index) {
              ...
              for (int i = 0; i < lineBarsData1.length; i++) {
                showingTooltipIndicators.add(
                  LineBarSpot(lineBarsData1[i], lineBarsData1.indexOf(lineBarsData1[i]),
            ...
            minX: 1,
            maxX: 32,
            maxY: ((maxList.reduce(max).toDouble() ~/ intervalLeftTiles) + 1 ) * intervalLeftTiles,
            minY: 0,
          );
  ...
  Future<List<dynamic>> _getExpenseDailySum() async {
    ...
    var url = Uri.parse(
      Config.API_URL + 'expense_sum_daily?strtDt=' + prevStrtDt + '&endDt=' + endDt
    );
        ...
        getTooltipItems: (List<LineBarSpot> lineBarsSpot) {
          return lineBarsSpot.map((lineBarSpot) {
            return LineTooltipItem(
              header[lineBarSpot.barIndex + 1] + ' ' + numberUtils.comma(lineBarSpot.y.toInt()),
              TextStyle(
