...
class _MyAssetBodyState extends State<MyAssetBody> {
  ...
  Widget build(BuildContext context) {  
    ...
    inputExchangeRateYn = inputExchangeRateYn == '' ? args.exchangeRateYn : inputExchangeRateYn;
  ...
  Widget _getPriceDivCdSelectBox() {
    return DropdownButton(
      ...
      items: ['', 'MANUAL', 'AUTO'].map<DropdownMenuItem<String>>((String value) {
        return DropdownMenuItem<String>(
          value: value,
          child: Text(value),
        );
...
class MyAssetParameter {
  ...
  MyAssetParameter({
    this.myAssetId = '', this.assetId = '', this.myAssetNm = '', this.ticker = '', this.priceDivCd = '',
    this.price = -1, this.qty = -1.0, this.exchangeRateYn = 'N', this.isInsert = true
  });
}