      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            Dropdown(),
            Consumer<Date>(
              builder: (_, date, __) => ExpenseChartBody(),
            ),
          ],
...
class ExpenseChartBody extends StatefulWidget {
  const ExpenseChartBody({Key? key}) : super(key: key);

  @override
  State<ExpenseChartBody> createState() => _ExpenseChartBodyState();
}

class _ExpenseChartBodyState extends State<ExpenseChartBody> {
    ...
    return FutureBuilder(
      ...
      builder: (BuildContext context, AsyncSnapshot snapshot) {
        ...
        // 데이터를 정상적으로 받아오게 되면 다음 부분을 실행하게 되는 것이다.
          ...
          var curExpensePrice = snapshot.data['data'][snapshot.data['data'].length - 1]['total_sum_price'];
          var prevExpensePrice = snapshot.data['data'][snapshot.data['data'].length - 2]['total_sum_price'];
              ...
              Card(
                ...
                child: Row(
                  children: [
                    Text('  저번달 보다 '),
                    Text(
                      numberUtils.comma((curExpensePrice - prevExpensePrice).abs()),
                      style: TextStyle(
                    ...
                    Text(
                      (curExpensePrice - prevExpensePrice > 0 ? ' 더' : ' 덜') + ' 썼어요'
                    )
                  ]
                )
              ),
